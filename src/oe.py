...
def download_file(source, destination, silent=False):
    try:
        ...
        if silent == False:
            ...
            download_dlg.create('AlexELEC', _(32181).encode('utf-8'), ' ', ' ')
    ...
    try:
        if tarfile.is_tarfile(filename):
            if silent == False:
                ...
                extract_dlg.create('AlexELEC ', _(32186).encode('utf-8'), ' ', ' ')
...
def copy_file(source, destination, silent=False):
    try:
        ...
        if silent == False:
            ...
            copy_dlg.create('AlexELEC', _(32181).encode('utf-8'), ' ', ' ')
...
def openWizard():
    ...
    try:
        winOeMain = oeWindows.wizard('service-AlexELEC-Settings-wizard.xml', __cwd__, 'Default', oeMain=__oe__)
        winOeMain.doModal()
        winOeMain = oeWindows.mainWindow('service-AlexELEC-Settings-mainWindow.xml', __cwd__, 'Default', oeMain=__oe__)  # None
    except Exception, e:
        xbmc.executebuiltin('Dialog.Close(busydialog)')
...
def openConfigurationWindow():
    ...
    try:
        winOeMain = oeWindows.mainWindow('service-AlexELEC-Settings-mainWindow.xml', __cwd__, 'Default', oeMain=__oe__)
        winOeMain.doModal()
        for strModule in dictModules:
...
def get_os_release():
    ...
    if os_release_info is not None:
        ...
        if 'ALEXELEC_ARCH' in os_release_info:
            architecture = os_release_info['ALEXELEC_ARCH']
        if 'ALEXELEC_BUILD' in os_release_info:
            build = os_release_info['ALEXELEC_BUILD']
        return (
            distribution,
            version,
            architecture,
